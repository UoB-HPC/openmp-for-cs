...
\begin{itemize}
  ...
  \item Use the \mintinline{fortran}|target| construct to transfer execution.
  \item Use the \mintinline{fortran}|map| clauses to transfer data.
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Compilers sometimes ignore different parts of the construct, depending on the situation.
...
%-------------------------------------------------------------------------------
\begin{frame}
\frametitle{How is this parallelism applied?}
Consider:
 \mintinline{fortran}|!$omp teams distribute|
 \begin{itemize}
  \item Loop iterations distributed between teams.
  \item Remember, you can’t synchronize between teams.
  \item So all iterations are independent.
  \item Implementations can, and will, share the work across the whole GPU:
  \begin{itemize}
     \item OpenMP teams being mapped to processing elements.
     \item Doesn’t matter how the work-items are grouped into work-groups (compute units) as no synchronisation.
  \end{itemize}
  \item Behaves somewhat like SIMD auto-vectorization.
\end{itemize}
\end{frame}
  %-------------------------------------------------------------------------------

\begin{frame}
\frametitle{What parallelism are you getting?}
\begin{itemize}
  \item With more than one possible mapping, sometimes you need to find out what is really happening.
  \item Compiler documentation:
  \begin{itemize}
    \item Cray: \mintinline{bash}|man intro_openmp|
  \end{itemize}
  \item Compiler output:
  \begin{itemize}
    \item In CCE 10, \mintinline{bash}|-fsave-loopmark flag|
  \end{itemize}

  \item Profiling:
  \begin{itemize}
  \item  \mintinline{bash}|$ nvprof --print-gpu-trace|
  \item  Look for the number of threads per block, and number of blocks.
  \item  Combine that with knowledge of pragma and number of loop iterations.
  \end{itemize}
  \end{itemize}
  \end{frame}
...
\begin{itemize}
\item Data transfers may happen at start and end of every \mintinline{fortran}|target| region.
...
\begin{itemize}
  ...
  \item Bulk transfers happen at beginning and end, not for every \mintinline{fortran}|target| region in the big loop.
...
\begin{frame}[fragile]
\frametitle{Update construct}
\begin{minted}[frame=single,fontsize=\tiny,linenos]{fortran}
!$omp target enter data map(to: A, B, C)
!$omp target
...
\begin{minted}[frame=single,linenos,fontsize=\tiny]{fortran}
...
\begin{itemize}
\item On the \mintinline{fortran}|target| construct, the pointers are evaluated every time.
\item The compiler/runtime makes sure the mappings occur as expected.
\item If use \mintinline{fortran}|map(from: )| then data transferred back to location specifed on \emph{entry} to region, even if pointers swapped.
\item Unlikely to run into confusing states if use \mintinline{fortran}|update| or \mintinline{fortran}|target exit data| to bring back the data.
\end{itemize}
...
\begin{itemize}
  ...
  \item Use the \mintinline{fortran}|target teams distribute parallel do| construct for parallelism.
  ...
  \item Try using the \mintinline{fortran}|collapse| clause to increase parallelism.
  \item Profile the code with \mintinline{bash}|nvprof| to explore the mapping.
  %\item Extra: Think about the performance compared to your CPU version.
\end{itemize}
\end{frame}
