...
\begin{itemize}
  ...
  !$omp end target
  \end{minted}
  \item Host execution idles until target region completes (exact semantics based on tasks).
\end{itemize}
...
\begin{itemize}
  ...
  \item Note, iterations that were assigned to the team by \mintinline{fortran}|distribute| construct are shared between threads in the team.
...
\begin{itemize}
  ...
  \item Allocate data on the device without copying from the host. It is uninitialised.
      Can later be copied back to the host (with \mintinline{fortran}|update| etc.) as long as allocated to host too.
    \begin{minted}{fortran}
    map(alloc: A)
    \end{minted}
\end{itemize}
...
\begin{itemize}
  ...
  \item Use the \mintinline{fortran}|update| construct to move the data explicitly between host and device.
