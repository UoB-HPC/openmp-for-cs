...
\begin{itemize}
  \item A \mintinline{fortran}|taskwait| also in Task B will ensure Task B waits for Tasks D and E to finish before finishing itself.
...
\begin{itemize}
  ...
  \vfill
  \item If used a \mintinline{fortran}|taskwait| before line 14, it would also wait for \emph{background} tasks. Also need to ensure wait on recursive task's descendants.
\end{itemize}
\end{column}
\end{columns}
...
\begin{itemize}
  \item \mintinline{fortran}|A| is shared.
  \item \mintinline{fortran}|B| is firstprivate.
  \item Any variables inside the subroutine are private.
\end{itemize}
