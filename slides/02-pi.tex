...
\begin{itemize}
  \item You can force yourself to specify everything manually by using the \mintinline{fortran}|default(none)| attribute.
...
\begin{itemize}
  ...
  \item A \mintinline{fortran}|critical| region only allows one thread to excute at any one time. No guarentees of ordering.
\end{itemize}
...
\begin{itemize}
  \item Each thread stores its partial sum, and the array is totally by the master thread serially at the end.
...
\begin{itemize}
  ...
  \item Therefore, cache line must be flushed to memory and reread into the other thread everytime.
