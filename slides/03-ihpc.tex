...
\begin{itemize}
  \item What happens when you run \mintinline{c}|malloc(sizeof(double)*N)|?
  \pause
...
\begin{itemize}
  \item \mintinline{bash}|OMP_PROC_BIND=false|: Often the default; threads may move! \mintinline{c}|proc_bind| clauses ignored.
  \item \mintinline{bash}|OMP_PROC_BIND=true|: Threads won't move, and follow \mintinline{c}|proc_bind| clauses or else the implementation default pinning.
...
\begin{itemize}
  \item 16 processes are spawned on one (or more) nodes according to the hostname list file given by the queuing system.
    \begin{itemize}
      \item E.g. with SLURM (\mintinline{bash}|sbatch|, etc.) set by \mintinline{bash}|$SLURM_JOB_NODELIST|.
      \item SLURM might spawn multiple processes depending on \mintinline{bash}|--ntasks-per-node|.
    \end{itemize}
...
\begin{itemize}
  ...
  \item Just pass in the OpenMP flag as usual:
    \begin{itemize}
      \item GNU: \mintinline{bash}|mpicc| -fopenmp
      \item Intel: \mintinline{bash}|mpiicc| -qopenmp
    \end{itemize}
...
\begin{itemize}
  ...
  \item A good idea to check \mintinline{c}|provided >= required|.
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item \mintinline{c}|MPI_THREAD_SINGLE| \\
  Only one thread will execute (no threads allowed).

  \item \mintinline{c}|MPI_THREAD_FUNNELED| \\
  May spawn threads, but only the original process may call MPI routines: the one that called \mintinline{c}|MPI_Init|.

  \item \mintinline{c}|MPI_THREAD_SERIALIZED| \\
  May spawn threads and any thread can make MPI calls, but only one at a time. \emph{Your} responsibility to synchronise.

  \item \mintinline{c}|MPI_THREAD_MULTIPLE| \\
...
Any thread can call MPI whenever it likes. The \mintinline{c}|MPI_THREAD_MULTIPLE| guarantees the MPI library will be OK with this.
\begin{minted}[frame=single]{c}
#pragma omp parallel
{
  ... // Parallel work
...
Intel and SLURM make it straightforward to run hybrid jobs, but need to use \mintinline{bash}|mpirun| not \mintinline{bash}|srun|.
